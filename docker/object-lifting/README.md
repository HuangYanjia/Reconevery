...
Reconevery runs it with the current Linux UID/GID, a read-only canonical input
mount, a writable stage-attempt mount, and one configured NVIDIA GPU. The
configuration-aware healthcheck creates a real CUDA raster context; image
existence alone is not reported as readiness.
